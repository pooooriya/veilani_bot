...
export class AppService {
  ...
  async SendVeilaniPool(): Promise<void> {
    Logger.log('Telegram Vote Sending at:', Date.now());
    const vote = await this.telegramService.SendVote();
    Logger.log('Telegram Vote Sent SuccessFully at:', Date.now());
    setTimeout(async () => {
      await this.telegramService.PinVote(vote.message_id);
    }, 2000);
    Logger.log('Telegram Vote SuccessFully Pinned at:', Date.now());
  }
